/**
 * Deletes all case models, cases, tasks and related instance data (all tenants).
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await client.query('DELETE FROM legal.instruments');
    await client.query('DELETE FROM legal.tasks');
    ...
    await client.query('DELETE FROM legal.case_model_task_models');
    ...
    const counts = await client.query<{ tbl: string; n: number }>(`
      ...
      UNION ALL SELECT 'tasks', count(*)::int FROM legal.tasks
    `);
