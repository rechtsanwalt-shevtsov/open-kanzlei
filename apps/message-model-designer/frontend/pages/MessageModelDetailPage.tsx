...
import {
  ...
  type SystemFieldRow,
} from '../lib/message-model-system-fields.js';
...
export function MessageModelDetailPage() {
  ...
  const [model, setModel] = useState<MessageModel | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    const [modelRes, modelAttrs, instanceAttrs] = await Promise.all([
      api.GET('/v1/message-models/{id}', { headers, params: { path: { id } } }),
      listMessageModelAttributes(id, locale, 'model'),
      listMessageModelAttributes(id, locale, 'instance'),
    ]);
    setLoading(false);
  ...
  const patchModel = useCallback(
    async (body: components['schemas']['UpdateMessageModelRequest']): Promise<boolean> => {
      if (!id) return false;
      setFieldSaving(true);
      setError(null);
      const res = await api.PATCH('/v1/message-models/{id}', {
  ...
  const allRows = useMemo((): ListRow[] => {
    ...
    const customRows: CustomFieldRow[] = instanceScopeItems.map((attribute) => ({
      kind: 'custom',
      id: attribute.id,
      attribute,
    }));
  ...
  const pageRangeLabel = msg('mmdPageRange')
  ...
  async function handleCreate(payload: AttributeDialogPayload): Promise<string | null> {
    ...
    const res = await createMessageModelAttribute(id, locale, payload);
  ...
  async function handleDeleteModel() {
    ...
    if (!window.confirm(msg('mmdDeleteModelConfirm'))) return;

    setDeletingModel(true);
    setError(null);
    const res = await api.DELETE('/v1/message-models/{id}', {
    ...
    navigate('/apps/message-model-designer');
  }
  ...
  const addLabel = activeTab === 'model' ? msg('mmdAddModelField') : msg('mmdAddTaskField');
  ...
  const platformLockFields = undefined;
          ...
          <header className="admin-page-header">
            ...
            <div className="admin-toolbar">
              ...
              <button
                ...
                title={msg('mmdDeleteModel')}
                aria-label={msg('mmdDeleteModel')}
              ...
              <Link
                to="/apps/message-model-designer/settings"
                ...
                {selectedIds.size > 0 && (
                  <div className="admin-list-actions-row">
                    ...
                    <span className="admin-list-toolbar-meta">
                      {msg('mmdSelectedCount').replace('{count}', String(selectedIds.size))}
                    </span>
                  </div>
                )}
              </div>
            ...
            <table className="admin-table admin-table--fixed-cols">
              <thead>
                <tr>
                  ...
                  {activeTab === 'model' && thSort('default', 'mmdColValue')}
                      ...
                      return (
                        <tr key={row.id} className="admin-table-row--system">
                          ...
                          <td>
                            ...
                            <span className="admin-table-sub"> ({msg('mmdSystemField')})</span>
                          </td>
                          {advanced && (
