...
import {
  MESSAGE_MODEL_DESIGNER_SETTINGS_SCHEMA,
...
function normalizeSettings(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, field] of Object.entries(MESSAGE_MODEL_DESIGNER_SETTINGS_SCHEMA)) {
    if (key in raw) {
...
export function AppSettingsPage() {
  ...
  const settingRows = useMemo(
    () => Object.entries(MESSAGE_MODEL_DESIGNER_SETTINGS_SCHEMA),
    [],
  );
  ...
  return (
    <div className="admin-page admin-page--shell">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        <Link to="/apps/message-model-designer">{msg('cmdAppTitle')}</Link>
        <span className="admin-breadcrumb-sep">›</span>
        <span aria-current="page">{msg('mmdSettings')}</span>
      </nav>

      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('mmdSettings')}</h1>
      </header>
          ...
          <div className="admin-list-card">
            <div className="admin-table-wrap">
              <table className="admin-table admin-table--settings">
                <thead>
                  <tr>
                    <th className="admin-settings-col-name">{msg('mmdSettingsColSetting')}</th>
                    <th className="admin-settings-col-default">{msg('mmdSettingsColDefault')}</th>
                    <th className="admin-settings-col-scope">{msg('mmdSettingsColFirm')}</th>
                    <th className="admin-settings-col-scope">{msg('mmdSettingsColUser')}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={key}>
                        ...
                        <td>
                          {isRecordSettingField(field) ? (
                            ...
                            <span className="admin-table-muted">{msg('mmdSettingsNotConfigurable')}</span>
                          )}
                        </td>
                        <td>
                          {isRecordSettingField(field) ? (
