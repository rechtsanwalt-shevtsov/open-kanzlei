...
import { messageModelStatusLabel } from '../lib/message-model-status.js';
...
export function MessageModelsListPage() {
  ...
  async function load() {
    setLoading(true);
    setError(null);
    const res = await api.GET('/v1/message-models', { headers: apiHeaders(locale) });
    if (res.error) {
  ...
  function modelLabel(m: MessageModel): string {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((m) => {
      ...
      const statusLabel = messageModelStatusLabel(m.status, msg).toLowerCase();
      return (
        label.includes(q) ||
        desc.includes(q) ||
  ...
  const sorted = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortColumn === 'status') {
        return (
          dir *
          messageModelStatusLabel(a.status, msg).localeCompare(
            messageModelStatusLabel(b.status, msg),
            locale,
          )
        );
      }
  ...
  async function handleBulkDelete() {
    ...
    if (!window.confirm(msg('mmdBulkDeleteConfirm'))) return;
    ...
    try {
      const results = await Promise.all(
        ids.map((id) =>
          api.DELETE('/v1/message-models/{id}', {
  ...
  return (
    <div className="admin-page admin-page--shell">
      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('mmdAppTitle')}</h1>
        <div className="admin-toolbar">
          <Link to="/apps/message-model-designer/new" className="button-outline">
            + {msg('mmdCreate')}
          </Link>
          <Link
          ...
          <div className="admin-table-wrap">
            <table className="admin-table admin-table--fixed-cols admin-table--case-models-list">
              ...
              <tbody>
                {pageItems.length === 0 ? (
                  <tr>
                    <td colSpan={colCount}>{msg('mmdEmpty')}</td>
                  </tr>
                ) : (
                  pageItems.map((m) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="cmd-col-name">
                          <Link to={`/apps/message-model-designer/${m.id}`}>{modelLabel(m)}</Link>
                        </td>
                        <td className="cmd-col-description">{m.description?.trim() || '—'}</td>
                        <td className="admin-table-col-status">
                          {messageModelStatusLabel(m.status, msg)}
                        </td>
                      </tr>
                    );
                  })
