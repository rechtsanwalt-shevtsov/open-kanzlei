...
import {
  TASK_MODEL_STATUSES,
  messageModelStatusLabel,
  type MessageModelStatus,
} from '../lib/message-model-status.js';
import type { SystemFieldRow } from '../lib/message-model-system-fields.js';
...
interface SystemFieldValueCellProps {
  ...
  model: MessageModel;
  saving: boolean;
  onPatch: (body: components['schemas']['UpdateMessageModelRequest']) => Promise<boolean>;
}
  ...
  if (row.fieldKey === 'status') {
    return (
      <select
        ...
        value={model.status}
        disabled={saving}
        onChange={(e) => void onPatch({ status: e.target.value as MessageModelStatus })}
