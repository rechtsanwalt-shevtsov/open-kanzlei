...
import { messageModelStatusLabel, type MessageModelStatus } from './message-model-status.js';

const SETTING_LABEL_KEYS: Record<string, MessageKey> = {
  defaultMessageModelStatus: 'mmdDefaultStatus',
  showTechnicalKeys: 'mmdShowKeys',
  defaultAttributeEncryptionMode: 'mmdDefaultAttributeEncryption',
  itemsPerPage: 'mmdItemsPerPage',
  editorLayout: 'mmdEditorLayout',
};

export function settingLabelKey(key: string): MessageKey {
  return SETTING_LABEL_KEYS[key] ?? 'mmdSettingsColSetting';
}
...
export function formatSettingValue(
  ...
  if (key === 'defaultMessageModelStatus' && typeof value === 'string') {
    return messageModelStatusLabel(value as MessageModelStatus, msg);
  }
  ...
  if (key === 'editorLayout' && value === 'simple') return msg('mmdEditorLayoutSimple');
  if (key === 'editorLayout' && value === 'advanced') return msg('mmdEditorLayoutAdvanced');
  return String(value);
}
