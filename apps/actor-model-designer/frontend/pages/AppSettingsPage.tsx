...
import {
  ACTOR_MODEL_DESIGNER_SETTINGS_SCHEMA,
...
function normalizeSettings(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, field] of Object.entries(ACTOR_MODEL_DESIGNER_SETTINGS_SCHEMA)) {
    if (key in raw) {
...
export function AppSettingsPage() {
  ...
  const settingRows = useMemo(
    () => Object.entries(ACTOR_MODEL_DESIGNER_SETTINGS_SCHEMA),
    [],
  );
  ...
  return (
    <div className="admin-page admin-page--shell">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-current="page">{msg('amdSettings')}</span>
      </nav>

      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('amdSettings')}</h1>
      </header>
          ...
          <div className="admin-list-card">
            <div className="admin-table-wrap">
              <table className="admin-table admin-table--settings">
                <thead>
                  <tr>
                    <th className="admin-settings-col-name">{msg('amdSettingsColSetting')}</th>
                    <th className="admin-settings-col-default">{msg('amdSettingsColDefault')}</th>
                    <th className="admin-settings-col-scope">{msg('amdSettingsColFirm')}</th>
                    <th className="admin-settings-col-scope">{msg('amdSettingsColUser')}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={key}>
                        ...
                        <td>
                          {isRecordSettingField(field) ? (
                            ...
                            <span className="admin-table-muted">{msg('amdSettingsNotConfigurable')}</span>
                          )}
                        </td>
                        <td>
                          {isRecordSettingField(field) ? (
