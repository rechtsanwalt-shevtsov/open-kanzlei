...
import {
  ...
  type SystemFieldRow,
} from '../lib/actor-model-system-fields.js';
...
export function ActorModelDetailPage() {
  ...
  const [model, setModel] = useState<ActorModel | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    const [modelRes, modelAttrs, instanceAttrs] = await Promise.all([
      api.GET('/v1/actor-models/{id}', { headers, params: { path: { id } } }),
      listActorModelAttributes(id, locale, 'model'),
      listActorModelAttributes(id, locale, 'instance'),
    ]);
    setLoading(false);
  ...
  const patchModel = useCallback(
    async (body: components['schemas']['UpdateActorModelRequest']): Promise<boolean> => {
      if (!id) return false;
      setFieldSaving(true);
      setError(null);
      const res = await api.PATCH('/v1/actor-models/{id}', {
  ...
  async function handleCreate(payload: AttributeDialogPayload): Promise<string | null> {
    ...
    const res = await createActorModelAttribute(id, locale, payload);
  ...
  async function handleDeleteModel() {
    ...
    if (!window.confirm(msg('amdDeleteModelConfirm'))) return;

    setDeletingModel(true);
    setError(null);
    const res = await api.DELETE('/v1/actor-models/{id}', {
    ...
    navigate('/apps/actor-model-designer');
  }
  ...
  const addLabel = activeTab === 'model' ? msg('amdAddModelField') : msg('amdAddActorField');
  ...
  return (
    <div className="admin-page admin-page--shell">
            ...
            <div className="admin-toolbar">
              ...
              {!model.is_system && (
                <button
                  ...
                  title={msg('amdDeleteModel')}
                  aria-label={msg('amdDeleteModel')}
                      ...
                      return (
                        <tr key={row.id} className="admin-table-row--system">
                          ...
                          <td>
                            ...
                            <span className="admin-table-sub"> ({msg('amdSystemField')})</span>
                          </td>
                          {advanced && (
      ...
      <AttributeDialog
        ...
        extendedFields
        lockFields={
          editTarget && isActorInstanceStatusAttribute(editTarget)
            ? {
                name: true,
                dataType: true,
                isRequired: true,
