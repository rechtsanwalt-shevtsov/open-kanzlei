...
export function CreateActorDialog({ open, models, onClose, onCreated }: CreateActorDialogProps) {
  const { locale, msg } = useI18n();
  const activeModels = useMemo(() => models.filter((m) => m.status === 'active'), [models]);
  const [actorModelId, setActorModelId] = useState('');
  ...
  useEffect(() => {
    if (!open) return;
    setActorModelId(activeModels[0]?.id ?? '');
    setStatus('');
    setInstanceFields([]);
    setFieldValues({});
    setMultiFieldValues({});
  ...
  useEffect(() => {
    ...
    void listActorModelAttributes(actorModelId, locale, 'instance').then((res) => {
      if (cancelled) return;
      ...
      const statusDef = findActorStatusDefinition(defs);
      const initialStatus =
  ...
  }, [open, actorModelId, locale]);

  const statusDefinition = useMemo(
    () => findActorStatusDefinition(instanceFields),
    [instanceFields],
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await api.POST('/v1/actors', {
  ...
  return (
    <div className="admin-dialog-backdrop" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="create-actor-dialog-title"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 id="create-actor-dialog-title">{msg('actCreate')}</h2>
        <form onSubmit={handleSubmit} className="form admin-dialog-form">
          {activeModels.length === 0 ? (
            <p className="form-error">{msg('actNoModels')}</p>
          ) : (
            <>
              <label>
                {msg('actColModel')}
              ...
              {!fieldsLoading && sortedFields.length > 0 && (
                <fieldset className="act-create-fields">
                  <legend>{msg('actFieldsSection')}</legend>
          ...
          <div className="admin-dialog-actions">
            ...
            >
              {submitting ? msg('loading') : msg('actCreate')}
            </button>
          </div>
        </form>
      </div>
