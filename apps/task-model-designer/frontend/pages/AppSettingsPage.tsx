...
import {
  TASK_MODEL_DESIGNER_SETTINGS_SCHEMA,
...
function normalizeSettings(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, field] of Object.entries(TASK_MODEL_DESIGNER_SETTINGS_SCHEMA)) {
    if (key in raw) {
...
export function AppSettingsPage() {
  ...
  const settingRows = useMemo(
    () => Object.entries(TASK_MODEL_DESIGNER_SETTINGS_SCHEMA),
    [],
  );
  ...
  return (
    <div className="admin-page admin-page--shell">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        <Link to="/apps/task-model-designer">{msg('cmdAppTitle')}</Link>
        <span className="admin-breadcrumb-sep">›</span>
        <span aria-current="page">{msg('tmdSettings')}</span>
      </nav>

      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('tmdSettings')}</h1>
      </header>
          ...
          <div className="admin-list-card">
            <div className="admin-table-wrap">
              <table className="admin-table admin-table--settings">
                <thead>
                  <tr>
                    <th className="admin-settings-col-name">{msg('tmdSettingsColSetting')}</th>
                    <th className="admin-settings-col-default">{msg('tmdSettingsColDefault')}</th>
                    <th className="admin-settings-col-scope">{msg('tmdSettingsColFirm')}</th>
                    <th className="admin-settings-col-scope">{msg('tmdSettingsColUser')}</th>
                  </tr>
                </thead>
                <tbody>
