...
import { taskModelStatusLabel } from '../lib/task-model-status.js';
...
export function TaskModelsListPage() {
  ...
  async function load() {
    setLoading(true);
    setError(null);
    const res = await api.GET('/v1/task-models', { headers: apiHeaders(locale) });
    if (res.error) {
  ...
  function modelLabel(m: TaskModel): string {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((m) => {
      ...
      const statusLabel = taskModelStatusLabel(m.status, msg).toLowerCase();
      return (
        label.includes(q) ||
        desc.includes(q) ||
  ...
  const sorted = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortColumn === 'status') {
        return (
          dir *
          taskModelStatusLabel(a.status, msg).localeCompare(
            taskModelStatusLabel(b.status, msg),
            locale,
          )
        );
      }
  ...
  async function handleBulkDelete() {
    ...
    if (!window.confirm(msg('tmdBulkDeleteConfirm'))) return;
  ...
  return (
    <div className="admin-page admin-page--shell">
      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('tmdAppTitle')}</h1>
        <div className="admin-toolbar">
          <Link to="/apps/task-model-designer/new" className="button-outline">
            + {msg('tmdCreate')}
          </Link>
          <Link
          ...
          <div className="admin-table-wrap">
            <table className="admin-table admin-table--fixed-cols admin-table--case-models-list">
              ...
              <tbody>
                {pageItems.length === 0 ? (
                  <tr>
                    <td colSpan={colCount}>{msg('tmdEmpty')}</td>
                  </tr>
                ) : (
                  pageItems.map((m) => {
