...
import {
  isTaskPlatformInstanceAttribute,
  isTaskReferencePlatformKey,
  orderedTaskPlatformDefinitions,
  type TaskPlatformInstanceKey,
...
import {
  ...
  type SystemFieldRow,
} from '../lib/task-model-system-fields.js';
...
type PlatformFieldRow = {
  kind: 'platform_field';
  id: string;
  platformKey: TaskPlatformInstanceKey;
  attribute: AttributeDefinition;
};

type ListRow = SystemFieldRow | CustomFieldRow | PlatformFieldRow;

function isCustomRow(row: ListRow): row is CustomFieldRow {
  return row.kind === 'custom';
}

function isPlatformFieldRow(row: ListRow): row is PlatformFieldRow {
  return row.kind === 'platform_field';
}

function platformFieldLabelKey(key: TaskPlatformInstanceKey): MessageKey {
  switch (key) {
    case 'status':
      return 'tmdInstanceStatus';
    case 'title':
      return 'tmdInstanceTitle';
    case 'weight':
      return 'tmdInstanceWeight';
    case 'due_date':
      return 'tmdInstanceDueDate';
    case 'predecessor_task_ids':
      return 'tmdInstancePredecessors';
    case 'dependent_task_ids':
      return 'tmdInstanceDependents';
  }
}

export function TaskModelDetailPage() {
  ...
  const [model, setModel] = useState<TaskModel | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    const [modelRes, modelAttrs, instanceAttrs] = await Promise.all([
      api.GET('/v1/task-models/{id}', { headers, params: { path: { id } } }),
      listTaskModelAttributes(id, locale, 'model'),
      listTaskModelAttributes(id, locale, 'instance'),
    ]);
    setLoading(false);
  ...
  const patchModel = useCallback(
    async (body: components['schemas']['UpdateTaskModelRequest']): Promise<boolean> => {
      if (!id) return false;
      setFieldSaving(true);
      setError(null);
      const res = await api.PATCH('/v1/task-models/{id}', {
  ...
  const allRows = useMemo((): ListRow[] => {
    ...
    const platformAttributes = orderedTaskPlatformDefinitions(instanceScopeItems);
    const customAttributes = instanceScopeItems.filter(
      (attribute) => !isTaskPlatformInstanceAttribute(attribute),
    );
    const rows: ListRow[] = platformAttributes.map((attribute) => ({
      kind: 'platform_field',
      id: attribute.id,
      platformKey: attribute.key as TaskPlatformInstanceKey,
      attribute,
    }));
    rows.push(
  ...
  function rowLabel(row: ListRow): string {
    ...
    if (row.kind === 'platform_field') return msg(platformFieldLabelKey(row.platformKey));
  ...
  const filtered = useMemo(() => {
    ...
    return allRows.filter((row) => {
      ...
      if (row.kind === 'platform_field') {
        return [rowLabel(row), row.attribute.key, msg('cmdFieldTypeEnum')]
          .join(' ')
          .toLowerCase()
          .includes(q);
      }
  ...
  const sorted = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (
        a.kind === 'system' ||
        b.kind === 'system' ||
        a.kind === 'platform_field' ||
        b.kind === 'platform_field'
      ) {
        return 0;
      }
  ...
  const pageRangeLabel = msg('tmdPageRange')
  ...
  async function handleCreate(payload: AttributeDialogPayload): Promise<string | null> {
    ...
    const res = await createTaskModelAttribute(id, locale, payload);
  ...
  async function handleDeleteModel() {
    ...
    if (!window.confirm(msg('tmdDeleteModelConfirm'))) return;

    setDeletingModel(true);
    setError(null);
    const res = await api.DELETE('/v1/task-models/{id}', {
    ...
    navigate('/apps/task-model-designer');
  }
  ...
  const addLabel = activeTab === 'model' ? msg('tmdAddModelField') : msg('tmdAddTaskField');
          ...
          <header className="admin-page-header">
            ...
            <div className="admin-toolbar">
              ...
              <button
                ...
                title={msg('tmdDeleteModel')}
                aria-label={msg('tmdDeleteModel')}
              ...
              <Link
                to="/apps/task-model-designer/settings"
                ...
                {selectedIds.size > 0 && (
                  <div className="admin-list-actions-row">
                    ...
                    <span className="admin-list-toolbar-meta">
                      {msg('tmdSelectedCount').replace('{count}', String(selectedIds.size))}
                    </span>
                  </div>
                )}
              </div>
            ...
            <table className="admin-table admin-table--fixed-cols">
              <thead>
                <tr>
                  ...
                  {activeTab === 'model' && thSort('default', 'tmdColValue')}
                      ...
                      return (
                        <tr key={row.id} className="admin-table-row--system">
                          ...
                          <td>
                            ...
                            <span className="admin-table-sub"> ({msg('tmdSystemField')})</span>
                          </td>
                          {advanced && (
                          ...
                          <td>
                            {isTaskReferencePlatformKey(row.platformKey) ? (
