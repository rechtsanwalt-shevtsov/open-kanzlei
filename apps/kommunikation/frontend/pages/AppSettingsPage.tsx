...
function normalizeSettings(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, field] of Object.entries(KOMMUNIKATION_SETTINGS_SCHEMA)) {
    if (key in raw) {
...
export function AppSettingsPage() {
  ...
  const settingRows = useMemo(
    () => Object.entries(KOMMUNIKATION_SETTINGS_SCHEMA),
    [],
  );
  ...
  return (
    <div className="admin-page admin-page--shell">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-current="page">{msg('actSettingsTitle')}</span>
      </nav>

      <header className="admin-page-header">
        <h1 className="admin-page-title">{msg('actSettingsTitle')}</h1>
      </header>
          ...
          <div className="admin-list-card">
            <div className="admin-table-wrap">
              <table className="admin-table admin-table--settings">
                <thead>
                  <tr>
                    <th className="admin-settings-col-name">{msg('actSettingsTitleColSetting')}</th>
                    <th className="admin-settings-col-default">{msg('actSettingsTitleColDefault')}</th>
                    <th className="admin-settings-col-scope">{msg('actSettingsTitleColFirm')}</th>
                    <th className="admin-settings-col-scope">{msg('actSettingsTitleColUser')}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={key}>
                        ...
                        <td>
                          {isRecordSettingField(field) ? (
                            ...
                            <span className="admin-table-muted">{msg('actSettingsTitleNotConfigurable')}</span>
                          )}
                        </td>
                        <td>
                          {isRecordSettingField(field) ? (
