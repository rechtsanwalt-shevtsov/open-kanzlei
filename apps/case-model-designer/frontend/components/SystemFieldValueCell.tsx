...
import {
  CASE_MODEL_STATUSES,
  caseModelStatusLabel,
  type CaseModelStatus,
} from '../lib/case-model-status.js';
import type { SystemFieldRow } from '../lib/case-model-system-fields.js';
...
interface SystemFieldValueCellProps {
  ...
  model: CaseModel;
  saving: boolean;
  onPatch: (body: components['schemas']['UpdateCaseModelRequest']) => Promise<boolean>;
}
  ...
  if (row.fieldKey === 'status') {
    return (
      <select
        ...
        value={model.status}
        disabled={saving}
        onChange={(e) => void onPatch({ status: e.target.value as CaseModelStatus })}
